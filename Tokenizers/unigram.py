"""
Tune SentencePiece Unigram hyperparameters for Sinhala.
...
Expected folder layout:
    project_root/
        pipeline_step12_backslashes_cleaned.jsonl
        tokenizers/
            unigram.py   <- this script
...
def build_param_combinations(grid: dict) -> list:
    ...
    [{"vocab_size": 8000, "max_sentencepiece_length": 16, "num_sub_iterations": 3}, ...]
    """
    keys = list(grid.keys())
...
# ---------------------------------------------------------------------------
# STEP 1: Train one tokenizer given a single set of param values.
# ---------------------------------------------------------------------------
def now_str() -> str:
...
def train_candidate(train_path: str, params: dict, out_prefix: str):
    spm.SentencePieceTrainer.train(
        ...
        num_sub_iterations=params["num_sub_iterations"],
...
def main():
    ...
    for i, params in enumerate(param_combinations, 1):
        tag = f"v{params['vocab_size']}_l{params['max_sentencepiece_length']}_i{params['num_sub_iterations']}"
        out_prefix = os.path.join(args.out_dir, f"unigram_{tag}")
    ...
    # -----------------------------------------------------------------
    print("\n" + "=" * 115)
    print(f"{'config':<20}{'vocab':<8}{'max_len':<9}{'sub_it':<8}{'tok/sent':<10}"
          f"{'chars/tok':<11}{'unk_%':<9}{'vocab_util_%':<14}{'roundtrip_%':<12}{'train_s':<8}")
    print("-" * 115)
    for r in results:
        print(f"{r['config']:<20}{r['vocab_size']:<8}{r['max_sentencepiece_length']:<9}{r['num_sub_iterations']:<8}"
